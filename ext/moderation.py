...
class Moderation(commands.Cog):
  ...
  async def ban(self, ctx, member: discord.Member = None, *, reason: str = None):
    if member == None:
      await ctx.reply(embed=generate_embed("An error occured", "You need to provide a member to ban."), mention_author=False)
    elif reason == None:
      await member.ban(reason="No reason provided.")
      await ctx.reply(f"Banned {member} for no reason.", mention_author=False)
    else:
  ...
  async def kick(self, ctx, member: discord.Member = None, *, reason: str = None):
    if member == None:
      await ctx.reply(embed=generate_embed("An error occured", "You need to provide a member to kick."), mention_author=False)
    elif reason == None:
      await member.kick(reason="No reason provided.")
      await ctx.reply(f"Kicked {member} for no reason.", mention_author=False)
    else:
