...
class Utility(commands.Cog):
  ...
  async def uptime(self, ctx: commands.Context):
    ...
    await ctx.reply(embed=discord.Embed(title="Current Uptime", description=f"Uptime: {days}d, {hours}h, {minutes}m, {seconds}s\n\nStartup Time: <t:{timestamp}:F>", color=0x88FF44))
  ...
  async def prefix(self, ctx: commands.Context):
    ...
    for prefix in await self.bot.get_prefix(ctx.message):
      if prefix == "":
        prefix_list += "No prefix enabled!"
      prefix_list += f"{prefix}\n"
    await ctx.reply(embed=discord.Embed(title="Your prefixes", description=prefix_list))
  ...
  async def add(self, ctx: commands.Context, *, prefix: str):
    ...
    if prefix in banned_prefixes:
      return await ctx.message.add_reaction(ctx.emoji.x)
    if len(prefix) >= 15:
      return await ctx.reply("Your prefix can't be longer than 15 characters, sorry!")
  ...
  async def invite(self, ctx: commands.Context):
    normal_perms = discord.utils.oauth_url(self.bot.user.id, discord.Permissions(27494536309),scopes=('bot', 'applications.commands'))
    moderation_perms = discord.utils.oauth_url(self.bot.user.id, discord.Permissions(274945363015),scopes=('bot', 'applications.commands'))
    await ctx.reply(embed=discord.Embed(title="Invite me using these links!", description=f"[Normal Permissions]({normal_perms})\n\n[Moderation Permissions]({moderation_perms})\n(Enables Moderation commands)", color=0x28e8ed))
  ...
  async def quote(self, ctx: commands.Context, message: discord.Message = None):
    ...
    await ctx.reply(embed=discord.Embed(title=f"{message.author} sent:", description=f"> {message.content}\n- {message.author.mention}"))
  ...
  async def ping(self, ctx: commands.Context):
    ...
    embed.add_field(name="<a:typing:990387851940229120> | Typing", value=f"```py\n{duration:.2f}ms```")
    embed.add_field(name="<a:loading2:990387928213631046> | Websocket", value=f"```py\n{websocket:.2f}ms```")
    embed.set_footer(text=f"Command ran by {ctx.author.display_name}", icon_url=ctx.author.display_avatar)
  ...
  async def counter(self, ctx: commands.Context):
    ...
    await ctx.reply(embed=discord.Embed(title="Command Counter", description=f"Commands used since last restart: {counter}", color=0x1f84c5))
  ...
  async def support(self, ctx: commands.Context):
    await ctx.reply(embed=discord.Embed(title="Support", description="Join my [support server]({self.bot.support_server})!"))
  ...
  async def suggest(self, ctx: commands.Context, *, suggestion: str = None):
    ...
    message = await channel.send(embed=discord.Embed(title=f"Suggestion by {ctx.author.name}", description=f"`{arg}`", color=0xFFFFFF))
  ...
  async def coinflip(self, ctx):
    ...
    await message.edit(content=f"It's {choice}!", allowed_mentions=discord.AllowedMentions.none())
  
  @commands.command()
  async def choose(self, ctx: commands.Context, choices: commands.Greedy[typing.Union[str, int]]):
