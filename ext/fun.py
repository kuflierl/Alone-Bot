...
class Fun(commands.Cog):
    ...
    @commands.command()
    async def token(self, ctx: commands.Context, *, member: discord.Member = None):
        member = member or ctx.author
        time = base64.b64encode(str(int(datetime.utcnow().timestamp()) - 12938400000).encode("utf-8")).decode().strip("==")
        encoded_id = base64.b64encode(str(member.id).encode("utf-8")).decode()
    ...
    async def meme(self, ctx: commands.Context):
        ...
        meme = random.choice([post async for post in subreddit.new(limit=100)])
